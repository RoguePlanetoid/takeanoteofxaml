...
internal class ApplicationProvider : IApplicationProvider
{
    ...
    {
        Content.Note = new();
        var result = await Upsert(GetUpsert(new_note_title, Content.Note));
    ...
    private async Task EditAsync(NoteModel? model)
    {
        if (model != null)
        {
            ...
            var result = await Upsert(GetUpsert(edit_note_title, Content.Note));
